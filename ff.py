...
class FFTeamPlayer(object):
    ...
    def __init__(
        ...
        if db_entry != None:
            ...
            self.in_team = (
                db_entry.get_current_squad_num() >= 0
                and db_entry.get_current_squad_num() < 11
            )
    ...
    def __str__(self):
        return "{} {}. {} {} {}".format(
            self.manager, self.squad_num, self.pos, self.club, self.name
        )
...
class FFTeam(object):
    ...
    preferred_pos = OrderedDict(
        [
            (ffdb.FFDBPlayer.GK, [0]),
    ...
    def __init__(
        ...
        if self.db == None and manager != None:
            ...
            query = ffdb.FFDBTeam.query().filter(
                ndb.AND(ffdb.FFDBTeam.year == year, ffdb.FFDBTeam.manager == manager)
            )
            teams = query.fetch(1)
        ...
        if self.db == None and create:
            ...
            raise KeyError(
                "No database entry for userid '{}', " "mgr '{}'".format(userid, manager)
            )
    ...
    def populate_players(self):
        ...
        query = ffdb.FFDBTeamPlayer.query().filter(
            ndb.AND(
                ffdb.FFDBTeam.year == self.year, ffdb.FFDBTeam.userid == self.userid
            )
        )
        ...
        if not done:
            raise TeamError(
                "Swapping players {} ({}) and {} ({}) invalid".format(
                    n1, self.players[n1].pos, n2, self.players[n2].pos
                )
            )
    ...
    def can_sub(self, n1, n2):
        ...
        return (
            ...
            and min(n1, n2) < 11
            and max(n1, n2) >= 11
            and not self._is_empty(max(n1, n2))
        )
    ...
    def add_player(self, player, price, max_squad_num=SQUADSIZE, check_clubs=True):
        """
        ...
        if self.funds < price:
            raise TeamError(
                "Player costs {} but only {} available".format(price, self.funds)
            )

        if check_clubs and not self.club_free(player.club):
            raise TeamError("No slots free for club '{}'".format(player.club))

        squad_num = self.find_free_squad_num(player.pos, max_squad_num=max_squad_num)

        # Check if this player has already been in the team and if so create
        # a new instance
        instance = 0
        while (
            ffdb.FFDBTeamPlayer.load(
                self.year, self.userid, player.player_key, instance
            )
            != None
        ):
            instance += 1
        ...
        self.players[squad_num] = FFTeamPlayer(
            player=player, team=self, price=price, squad_num=squad_num
        )
...
class FFLeague(object):
    ...
    def __init__(self, year, populate_teams=True):
        ...
        query = ffdb.FFDBTeam.query().order(
            -ffdb.FFDBTeam.total_score, -ffdb.FFDBTeam.funds
        )
        query = query.filter(ffdb.FFDBTeam.year == year)

        for i, team in enumerate(query):
            self.teams[team.manager] = FFTeam(
                year=year,
                userid=team.userid,
                db_entry=team,
                position=i + 1,
    ...
    def __str__(self):
        ...
        return "League {} contains {} teams".format(self.year, len(self.teams))
...
class FFPlayer(object):
    ...
    def __init__(self, db_entry, manager=None):
        ...
        self.url = "http://www.fantasyleague.com/Pro/Stats/Player/" + "{}.aspx".format(
            db_entry.url
        )
        self.img_url = (
            "http://www.fantasyleague.com/sharedassets/"
...
class FFPlayerList(object):
    ...
    def __init__(self, year, pos=None, club=None, unowned=False, player_key=None):
        """
        ...
        if player_key != None:
            ...
            # this key
            tpquery = ffdb.FFDBTeamPlayer.query().filter(
                ndb.AND(
