...
class FFPage(TemplateView):
    def common_setup(self, menu_num=0, year=None, league=None, populate_teams=False):
        ...
        if not hasattr(self, "template_values"):
            ...
            self.year = year
            if year == None:
                self.year = self.request.get("year", ffdb.FFDBSeason.current_season())
            self.league = league
            if league == None:
                self.league = ff.FFLeague(self.year, populate_teams=populate_teams)

            # Create ordered dictionaries for each element in the main menu
            # 0. League. The league table
            league_menu = OrderedDict(
                [
                    ("League Table", "./league.html"),
            ...
            # 4. Rules
            rules_menu = OrderedDict(
                [("Rules", "./rules.html"), ("Past Seasons", "./history.html")]
            )
    ...
    def render_from_cache(self, cache_name):
        ...
        if html != None:
            ...
            output = re.sub('\<a class="loginurl".*\<\/a\>', login_string, html)
    ...
    def render(self, page_name, cache_name=None, respond=True):
        ...
        if cache_name != None:
            ...
            if expiry < 0:
                expiry += 24 * 60 * 60
            logging.info(
                "Caching page {} to expire in {} seconds".format(cache_name, expiry)
            )
    ...
    def get(self):
        ...
        self.add_tab(
            Tab(
                "Goalkeepers",
                ffdb.FFDBPlayer.GK,
                "./playerlisttab.html?pos={}".format(ffdb.FFDBPlayer.GK),
            )
        )
        self.add_tab(
            Tab(
                "Fullbacks",
                ffdb.FFDBPlayer.FB,
                "./playerlisttab.html?pos={}".format(ffdb.FFDBPlayer.FB),
            )
        )
        self.add_tab(
            Tab(
                "Centrebacks",
                ffdb.FFDBPlayer.CB,
                "./playerlisttab.html?pos={}".format(ffdb.FFDBPlayer.CB),
            )
        )
        self.add_tab(
            Tab(
                "Midfielders",
                ffdb.FFDBPlayer.M,
                "./playerlisttab.html?pos={}".format(ffdb.FFDBPlayer.M),
            )
        )
        self.add_tab(
            Tab(
                "Strikers",
                ffdb.FFDBPlayer.S,
                "./playerlisttab.html?pos={}".format(ffdb.FFDBPlayer.S),
            )
        )
...
class PlayerListTab(FFPage):
    ...
    def display_page(self, year, pos, respond=True):
        ...
        self.set_title("Player List")
        self.render(
            self.TEMPLATE, cache_name=self.create_cache_name(pos), respond=respond
        )
...
class Player(FFPage):
    ...
    def get(self):
        ...
        if player_key != None:
            player = ff.FFPlayerList(self.year, player_key=player_key).find_player(
                player_key
            )
        else:
            player = None
...
class ViewTeams(FFPage):
    def get(self):
        ...
        for team in self.league:
            ...
            submenu[team.manager] = "./teams.html?manager={}".format(team.safemgr)
...
class AddTeam(FFPage):
    ...
    def validate_name(self, manager, userid):
        ...
        if team != None:
            raise ValueError("Manager name '{}' already exists".format(manager))
    ...
    def post(self):
        ...
        try:
            ...
            team = ff.FFTeam(year, self.user.user_id(), manager=manager, create=True)
    ...
    def get(self):
        ...
        if action == "changeTeamName":
            ...
            try:
                ...
                success = True
                result = "Created new seasion {}: {} - {}".format(self.year, start, end)
            ...
            if pos not in ff.FFLeague.pos_list():
                ...
                result = "Error: Player with key {} already exists".format(player_key)
            else:
            ...
            if self.league.season.enableUpdates == 0:
                ...
                try:
                    tools.update_player_scores(self.league.season, log, pos=pos)
                except Exception as e:
                    ...
                    success = True
                    result = "Updating scores for position '{}' complete".format(pos)
            ...
            if self.is_float(price_string):
                ...
                price = None
            in_player = ff.FFPlayerList(
                self.year, player_key=in_player_key
            ).find_player(in_player_key)
            team = self.league.get_team(manager=manager)
            if team != None:
            ...
            if (
                manager == None
                or in_player_key == None
                or out_player_key == None
                or price == None
            ):
                ...
                # club
                sn = team.add_player(in_player, price, check_clubs=not replace_non)
            ...
            for team in self.league:
                ...
                log.append("Added {}M to {}'s funds".format(new_funds, team.manager))
            success = True
        ...
        logging.info("Performed action '{}'. Result: '{}'".format(action, result))

        self.response.headers["Content-Type"] = "application/json"
        self.response.out.write(
            json.dumps({"success": success, "result": result, "log": log})
        )
...
class Auction(FFPage):
    def get(self):
        ...
        possible_pos_list = []
        for i in xrange(11):
            possible_pos_list.append((i, "/".join(ff.FFTeam.valid_position_list(i))))
        ...
        self.set_value("club_list", self.league.club_list(self.year))
        self.add_tab(Tab("Auction", "auction"))
        self.add_tab(Tab("Available Players", "available", "./freeplayers.html"))
...
class GetPlayersFromClub(FFPage):
    ...
    def get(self):
        ...
        playerlist = ff.FFPlayerList(self.year, pos=pos, club=club, unowned=unowned)
        ...
        self.response.headers["Content-Type"] = "application/json"
        self.response.out.write(json.dumps({"players": players, "managers": managers}))
    ...
    def get(self):
        ...
        if not self.admin:
            ...
            error = "Error: '{}' already belongs to '{}'".format(
                player.name, player.manager
            )
        else:
            # The parameters seem OK so try adding a player to the team
            try:
                squad_num = team.add_player(
                    player, float(price), max_squad_num=max_squad_num
                )
            except ff.TeamError as e:
        ...
        output = {
            "result": result,  # Result of this operation
            "error": error,  # error string if anything went wrong
            "squad_num": squad_num,
        }  # squad number if call succeeded
        self.response.headers["Content-Type"] = "application/json"
