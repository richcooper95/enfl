    ...
    def update_properties(self, properties):
        ...
        for field, new in properties.items():
            ...
            if new != None and new != old:
                logging.debug(
                    "{}. {} changed from {} to {}".format(str(self), field, old, new)
                )
                changed = True
            else:
...
class FFDBTeam(FFDBEntry):
    ...
    def set_points_and_pos(self, week, diff_scored=0, diff_missed=0, position=None):
        """
        ...
        if len(self.weeks) == 0:
            self.weeks.append(FFDBTeamWeek(total_points=0, total_missed=0, position=0))
    ...
    def update(self, name=None, funds_diff=0):
        ...
        or False otherwise
        """
        return self.update_properties({"name": name, "funds": self.funds + funds_diff})
...
class FFDBTeamPlayer(FFDBEntry):
    ...
    squad_num = ndb.IntegerProperty()
    weeks = ndb.LocalStructuredProperty(FFDBTeamPlayerWeek, repeated=True)
    ...
    def get_key(year, userid, player_key, instance):
        return ndb.Key(
            "FFDBTeamPlayer",
            "{}-{}-{}{}".format(
                year, player_key, userid, instance if instance != 0 else ""
            ),
        )
    ...
    def save(self):
        self.key = self.get_key(self.year, self.userid, self.player_key, self.instance)
        self.put()
...
class FFDBPlayer(FFDBEntry):
    ...
    status = ndb.StringProperty(
        choices=[FIT, INJURED, SUSPENDED, DOUBTFUL, INELIGABLE, LFT, INTERNATIONAL]
    )
...
class FFDBClub(FFDBEntry):
    ...
    def update(self, club):
        ...
        """
        return self.update_properties({"name": club.name, "url": club.url})
